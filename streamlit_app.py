...
@st.cache_data
def load_data():
    df = pd.read_csv("https://raw.githubusercontent.com/Mainak-dev/cdsco-sec-keyword-search/main/sec_text_dump.csv")
    df["FullText"] = df["FullText"].astype(str).str.lower()
    return df
...
query = st.text_input("Enter a keyword to search (e.g., Gepotidacin, Cipla, Trial):").lower().strip()

if query:
    hits = df[df["FullText"].str.contains(query, na=False)]
